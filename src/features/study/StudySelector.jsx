// src/features/study/StudySelector.jsx
...
export default function StudySelector({
  ...
  const decksArr = getDeckArray(repo);
    ...
    <div className="mb-4 rounded-lg border border-slate-200 bg-white p-4">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <label className="block flex-1">
          ...
          <select
            multiple
            size={Math.min(8, decksArr.length || 4)}
            className="w-full border rounded px-2 py-2"
            value={activeIds}
            onChange={(e) => {
              const opts = Array.from(e.target.selectedOptions).map((o) => o.value);
              onChangeActiveIds(opts);
            }}
          >
            {decksArr.map((d) => (
        ...
        <div className="flex gap-2">
          <button className="px-3 py-2 rounded border" onClick={() => onChangeActiveIds([])}>
            Clear
          </button>
          <button
            className="px-3 py-2 rounded bg-emerald-600 text-white disabled:opacity-50"
            disabled={activeIds.length === 0}
            onClick={onLoadToStudy}
          >
            Load to Study
          </button>
        </div>
